...
get_suscriptions_to_add_or_update <- function(con, email, lista_customer_ids, datos_bd, datos_suscripciones) {
    ...
    message("entro a get_suscriptions")
...
syncStripeDBProcessLight <- function() {
    limit_request_stripe = 100
    ...
    # lista de emails que se van a tratar (porque esten ligados a suscripciones manuales o porque se han creado suscripciones recientemente en stripe sobre estos emails)
    lista_emails_unicos = c()


    message("empiezo")
    # primero sacamos todas las suscripciones con fecha creacion > current_date - 3 (suscripciones que se crearon como maximo hace 3 dias, por si se cayera el servidor que ejecuta el script varios dias)
    ...
    suscripcion_url = sprintf("https://api.stripe.com/v1/subscriptions?limit=%d&status=%s&created[gt]=%s", limit_request_stripe, "all", start_point)
    ...
    if(!is.null(total_suscripciones) && length(total_suscripciones$data)>0 && nrow(total_suscripciones$data)>0){
        ...
        if(!is.null(datos_suscripciones) && !is.null(datos_suscripciones$id) && length(datos_suscripciones$id)>0){
            # si hay alguna suscripcion creada en los ultimos dias
            message("entro")
            # sacamos la lista de customers
            ...
            # primero, nos quedamos con los customer ids que tienen alguna suscripcion recientemente creada
            # lista_cust_ids_a_tratar = datos_clientes[datos_clientes$id %in% datos_suscripciones$id, "id"]
            lista_emails_a_tratar = datos_clientes[datos_clientes$id %in% datos_suscripciones$customer, "email"]
            message("lista emails a tratar")
            message(lista_emails_a_tratar)

            # ahora sacamos para cada email todos los customer ids que hay
            lista_emails_unicos = unique(lista_emails_a_tratar)
            ...
            for(email in lista_emails_unicos){
                ...
                lista_customer_ids = datos_clientes[datos_clientes$email==email & (datos_clientes$id %in% datos_suscripciones$customer), "id"]
                message(lista_customer_ids)
                if(length(lista_customer_ids)>0){
        ...
        # vemos el tema permisos


    }else {
        message("no hay suscripciones creadas recientemente en stripe")
    }
    ...
    # ver la lista de emails que se han podido cambiar desde la parte de stripe, o la lista de emails que se ha podido cambiar desde las manuales
    # iterar unciamente sobre estos emails sobre la sincronizacion con keycloak
    message("llego a keycloak")
    # message(lista_emails_unicos) # ya lo tenemos para las que vengan de stripe
    # lista_emails_unicos = c("ivan.lopez@stratebi.com", "pedro@gmail.com")
    # sacamos los id psicologo de estos emails, a la vez que de las suscripciones manuales que tengan una fecha inicio de hace 3 dias en adelante.
    emails_a_tratar_text = paste("'",lista_emails_unicos,"'", sep="", collapse = ", ")
    query <- sprintf("SELECT id, email, rol
        from psicologo 
        where email in (%s)
        group by 1,2
        ...
        select p.id as id, p.email as email, p.rol as rol
        from SUSCRIPCION s
        inner join psicologo p on p.id=s.fk_psicologo
        where s.tipo_suscripcion='manual'
        and cast(s.fecha_inicio as date) > current_date -3
        group by 1,2", emails_a_tratar_text)
    # message(query)
    ...
    if(!is.null(datos_psicologos) && length(datos_psicologos$id)>0){
        ...
        # message(datos_psicologos)
        # message(datos_psicologos[datos_psicologos$id==2, "email"])
        # # datos_psicologos = data.frame(datos_psicologos)
        # message(datos_psicologos[datos_psicologos$id==2, "email"])
    ...
    # este proceso sacará las suscripciones que se hayan creado hace 3 dias a lo sumo 
}